...
public class demo1388 {
    //1388. 3n 块披萨
    //动态规划
    public int maxSizeSlices(int[] slices) {
        int[] v1 = new int[slices.length - 1];
        int[] v2 = new int[slices.length - 1];
        System.arraycopy(slices, 1, v1, 0, slices.length - 1);//去掉第一个
        System.arraycopy(slices, 0, v2, 0, slices.length - 1);//去掉最后一个
        int ans1 = calculate(v1);
        int ans2 = calculate(v2);
        return Math.max(ans1, ans2);//分别考虑不选头尾的情况
    }
